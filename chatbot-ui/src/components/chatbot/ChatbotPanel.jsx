...
import { MessagesSquare, MessageCircle, RefreshCcw, History, Plus, ChevronLeft, X, MoreVertical, Trash2 } from 'lucide-react';
...
const ChatbotPanel = () => {
    ...
    // ── View State ──────────────────────────────────────────────────────────
    const [showHistory, setShowHistory] = useState(() => localStorage.getItem('crypto_show_history') === 'true');
    ...
    useEffect(() => {
        ...
        const isActualAccountSwitch = user?.id && prevUserIdRef.current && prevUserIdRef.current !== user.id;
    ...
    useEffect(() => {
        ...
        const clearUnread = () => {
            if (!showHistory && activeSessionId && !document.hidden) {
                ...
                localStorage.setItem(`crypto_viewed_${userPrefix}_${activeSessionId}`, Date.now().toString());
    ...
    const loadSessions = useCallback(async () => {
        ...
        try {
            ...
            setSessions(data.map(s => {
                const viewedAt = parseInt(localStorage.getItem(`crypto_viewed_${userPrefix}_${s.id}`)) || 0;
                ...
                const isUnread = s.last_message_at > viewedAt || localStorage.getItem(`crypto_unread_${userPrefix}_${s.id}`) === 'true';
                    ...
                    <div className="flex-1 overflow-y-auto w-full custom-scrollbar p-2">
                        {(loadingSessions || authLoading) ? (
                            <div className="flex flex-col items-center justify-center h-full text-indigo-500 gap-4 p-8">
                                ...
                                <p className="text-center font-medium text-slate-500 animate-pulse">Loading history...</p>
                            </div>
                        ) : sessions.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-4 p-8">
                                ...
                                <p className="text-center font-medium">
                                    No past conversations found.<br />Start a new chat to begin!
                                </p>
                            </div>
                        ) : (
                            <div className="flex flex-col gap-2 p-2" onClick={() => setMenuOpenId(null)}>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline mb-1">
                                                <h3 className={`font-bold ${session.isUnread ? 'text-indigo-900' : 'text-slate-800'}`}>CryptoBot</h3>
                                                <div className="flex items-center gap-2">
                                                    <span className={`text-[11px] ${session.isUnread ? 'font-bold text-indigo-500' : 'font-medium text-slate-400'}`}>
                                                        {session.time}
                                                    </span>
                                                    {!isGuest && (
                                            ...
                                            <p className={`text-sm truncate ${session.isUnread ? 'text-slate-900 font-semibold' : 'text-slate-600'}`}>
                                                {session.isEnded ? (
                                                    <span className="text-slate-900 font-semibold text-xs uppercase tracking-tight">Chat has ended</span>
                                                ) : (
                                                    session.preview
                                                )}
                                            </p>
...
const ActiveChat = ({ chatControls, defaultQuestions, onOpenHistory, showHistoryBtn, onNewChat }) => {
    const {
        isConnected, messages, isTyping, isSessionEnd,
        currentTranscript, sendMessage, sendAudioChunk,
        sendTranscribeRequest, sendTypingIndicator,
    } = chatControls;
        ...
        <>
            ...
            {!isConnected ? (
                <div className="flex-1 flex flex-col items-center justify-center text-indigo-500 gap-4 p-8">
                    <RefreshCcw className="w-12 h-12 animate-spin text-indigo-400 opacity-80" />
                    <p className="text-center font-medium text-slate-500 animate-pulse">Loading chats...</p>
                </div>
            ) : (
                    ...
                    {isSessionEnd ? (
                        <div className="p-8 text-center border-t border-slate-100 bg-white/80 backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <p className="text-slate-500 text-sm font-medium leading-relaxed">
                                Session ended due to inactivity.<br />
                                <button 
                                    onClick={onNewChat}
                                ...
                                <span className="text-slate-400"> to start a new conversation.</span>
                            </p>
                        </div>
                    ) : (
                        <>
