...
mcp = FastMCP("CryptoTradingServer")
...
def _fetch_multiple_prices(symbols: list[str]) -> dict[str, Decimal]:
    """Fetch prices for multiple symbols at once with caching."""
    now = time.time()
    results = {}
    to_fetch = []
    ...
    for sym in symbols:
        sym_upper = sym.upper()
        if sym_upper in _price_cache and now - _price_cache[sym_upper]['time'] < CACHE_TTL:
            results[sym_upper] = _price_cache[sym_upper]['price']
        else:
    ...
    params = {"symbol": ",".join(to_fetch), "convert": "USD"}
    ...
    for sym in to_fetch:
        coin_data = data["data"].get(sym)
        if coin_data:
            price = Decimal(str(coin_data[0]["quote"]["USD"]["price"]))
            results[sym] = price
            _price_cache[sym] = {'price': price, 'time': now}
        else:
            raise ValueError(f"Symbol '{sym}' not found on CoinMarketCap")

    return results

def _fetch_price(symbol: str) -> Decimal:
    """Internal helper: fetch raw price as Decimal for trade math. NOT an MCP tool."""
    return _fetch_multiple_prices([symbol])[symbol.upper()]


def _apply_slippage(price: Decimal, side: str, quantity: Decimal) -> Decimal:
    """BUY = slightly higher price, SELL = slightly lower price.
       Slippage scales with USD order value."""
    order_value = price * quantity
    
    # Tiered slippage based on USD value
    if order_value < Decimal("1000"):
        slip = Decimal("0.0002") # 0.02%
    elif order_value < Decimal("10000"):
        slip = Decimal("0.0005") # 0.05%
    elif order_value < Decimal("50000"):
        slip = Decimal("0.0010") # 0.10%
    else:
        slip = Decimal("0.0025") # 0.25%

    if side == "BUY":
        return (price * (1 + slip)).quantize(Decimal("0.00000001"))
    return (price * (1 - slip)).quantize(Decimal("0.00000001"))
...
def _get_portfolio_value(db, user_id: str) -> Decimal:
    ...
    try:
        prices = _fetch_multiple_prices(symbols)
        for pos in positions:
            sym_price = prices.get(pos['symbol'].upper(), Decimal("0"))
            if sym_price > 0:
                total += Decimal(str(pos['quantity'])) * sym_price
            else:
                total += Decimal(str(pos['total_cost_basis']))
    except Exception as e:
        logger.error(f"Failed to bulk fetch prices for portfolio: {e}")
...
def paper_buy(symbol: str, quantity: str, user_id: str = "") -> dict:
    ...
    try:
        ...
        fill_price = _apply_slippage(market_price, "BUY", qty)
        ...
        return {"error": "Failed to fetch current market price."}
...
def paper_sell(symbol: str, quantity: str, user_id: str = "") -> dict:
    ...
    try:
        market_price = _fetch_price(symbol)
        fill_price = _apply_slippage(market_price, "SELL", qty)
        subtotal = (qty * fill_price).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        fee = _calculate_fee(subtotal)
